...
export default function RentIncomeJournal() {
  ...
  useEffect(() => {
    fetch('http://localhost:8080/api/rent-income')
      .then(res => {
  ...
  const handleAddEntry = async (e) => {
    ...
    const newRecord = {
      ...formData,
      annualRates: parseNumber(formData.annualRates),
      total: monthsTotal
    };

    try {
      const res = await fetch('http://localhost:8080/api/rent-income', {
        method: 'POST',
        headers: {
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
        <div className="p-6 border-b border-slate-200">
          <h2 className="text-xl font-bold text-slate-800">Rent Income Calculation Journal</h2>
          <p className="text-slate-500 text-sm">ගෙවල් කුලී ආදායම් ගණනය කිරීමේ ජර්නලය - දත්ත ඇතුලත් කිරීම</p>
        </div>
        ...
        <form onSubmit={handleAddEntry} className="p-6 space-y-8">
          ...
          <div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              ...
              <InputField value={formData?.annualRates || ''} onChange={handleChange} label="Annual Rates" sinhalaLabel="වාර්ෂික තක්සේරුව" name="annualRates" type="number" />
            </div>
          </div>
          ...
          <table className="w-full text-sm text-left border-collapse min-w-max">
            <thead className="bg-slate-50 text-slate-700 text-xs uppercase text-center border-b border-slate-200">
              <tr>
                ...
                <th rowSpan="2" className="border border-slate-200 p-2 whitespace-nowrap">Description of Property/Building etc.<br/><span className="text-[10px] font-normal normal-case">දේපල/ගොඩනැගිලි ස්වභාවය</span></th>
                <th rowSpan="2" className="border border-slate-200 p-2 whitespace-nowrap">Annual Rates<br/><span className="text-[10px] font-normal normal-case">වාර්ෂික තක්සේරුව</span></th>
            ...
            <tbody>
              {records.length === 0 && (
                <tr>
                  <td colSpan="17" className="p-8 text-center text-slate-500 border border-slate-200">
              ...
              {records.map((record) => (
                <tr key={record.id} className="hover:bg-slate-50 even:bg-gray-50 transition-colors">
                  ...
                  <td className="border border-slate-200 px-2 py-1.5 text-right text-blue-600 font-medium">{record.annualRates}</td>
