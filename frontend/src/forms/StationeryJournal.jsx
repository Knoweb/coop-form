...
export default function StationeryJournal() {
  ...
  useEffect(() => {
    fetch('http://localhost:8080/api/stationery-journal')
      .then(res => {
  ...
  const handleAddEntry = async (e) => {
    ...
    try {
      const res = await fetch('http://localhost:8080/api/stationery-journal', {
        method: 'POST',
        headers: {
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
        <div className="p-6 border-b border-slate-200">
          <h2 className="text-xl font-bold text-slate-800">Stationery Expenditure Journal</h2>
          <p className="text-slate-500 text-sm">ලිපි ද්‍රව්‍ය වැය ජර්නලය - දත්ත ඇතුලත් කිරීම</p>
        </div>
        ...
        <form onSubmit={handleAddEntry} className="p-6 space-y-8">
          ...
          <div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 gap-4">
              ...
              <InputField value={formData?.sectionAndUnit || ''} onChange={handleChange} label="Section and Unit" sinhalaLabel="ස්ථානය සහ කාර්යාංශ" name="sectionAndUnit" />
              <InputField value={formData?.annualEstimate || ''} onChange={handleChange} label="Annual Estimate" sinhalaLabel="වාර්ෂික තක්සේරු වැය" name="annualEstimate" type="number" />
              <InputField value={formData?.pageNo21B || ''} onChange={handleChange} label="21. B Page No." sinhalaLabel="21 බී. පිටු අංකය" name="pageNo21B" />
            </div>
          </div>

          {/* Group 2: Monthly Expenditure */}
          <div>
            <h3 className="text-lg font-bold text-indigo-700 mb-4 border-b border-indigo-100 pb-2">Monthly Expenditure (මාසික වියදම)</h3>
          ...
          <table className="w-full text-sm text-left border-collapse min-w-max">
            <thead className="bg-slate-50 text-slate-700 text-xs uppercase text-center border-b border-slate-200">
              <tr>
                ...
                <th rowSpan="2" className="border border-slate-200 p-2 whitespace-nowrap">Section & Unit<br/><span className="text-[10px] font-normal normal-case">ස්ථානය සහ කාර්යාංශ</span></th>
                <th rowSpan="2" className="border border-slate-200 p-2 whitespace-nowrap">Annual Estimate<br/><span className="text-[10px] font-normal normal-case">වාර්ෂික තක්සේරු වැය</span></th>
                <th rowSpan="2" className="border border-slate-200 p-2 whitespace-nowrap">21. B Page No.<br/><span className="text-[10px] font-normal normal-case">21 බී. පිටු අංකය</span></th>
            ...
            <tbody>
              ...
              {records.map((record) => (
                <tr key={record.id} className="hover:bg-slate-50 even:bg-gray-50 transition-colors">
                  ...
                  <td className="border border-slate-200 px-2 py-1.5">{record.sectionAndUnit}</td>
                  <td className="border border-slate-200 px-2 py-1.5 text-right text-blue-600 font-medium">{record.annualEstimate}</td>
                  <td className="border border-slate-200 px-2 py-1.5 text-center">{record.pageNo21B}</td>
