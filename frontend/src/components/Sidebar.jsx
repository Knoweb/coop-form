  ...
  return (
    <div className={`bg-slate-900 text-slate-300 h-screen fixed left-0 top-0 flex flex-col shadow-2xl z-50 transition-all duration-300 ${isOpen ? 'w-64 translate-x-0' : 'w-64 -translate-x-full'}`}>
      ...
      <div className="flex-1 py-6 px-4 space-y-2 overflow-y-auto">
        ...
        <button
          onClick={() => { setCurrentForm('form15mMilk'); setIsOpen(false); }}
          className={`w-full flex items-center space-x-3 px-4 py-3 rounded-xl transition-colors text-left leading-tight ${currentForm === 'form15mMilk' ? 'bg-indigo-600/10 text-indigo-400 font-semibold' : 'hover:bg-slate-800 hover:text-slate-200'}`}
        >
          <FileText className="w-5 h-5 shrink-0" />
          <div className="flex flex-col">
            <span className="text-xs text-slate-400">කිරි මධ්යස්ථානය</span>
            <span>Form 15 M </span>
          </div>
        </button>
        <button
          onClick={() => { setCurrentForm('form16a'); setIsOpen(false); }}
          className={`w-full flex items-center space-x-3 px-4 py-3 rounded-xl transition-colors text-left leading-tight ${currentForm === 'form16a' ? 'bg-indigo-600/10 text-indigo-400 font-semibold' : 'hover:bg-slate-800 hover:text-slate-200'}`}
        >
          <FileText className="w-5 h-5 shrink-0" />
          <div className="flex flex-col">
            <span className="text-xs text-slate-400">තොග ගබඩාව සටහන</span>
            <span>Form 16 A</span>
          </div>
        </button>
        <button
